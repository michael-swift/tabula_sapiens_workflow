...
# exit upon a command e.g. BLAST makeblastdb failing
set -e
...
do
    ...
    do
        ...
        for SEGMENT in C
        do
            cat ${GERMDIR}/${SPECIES}/constant/imgt_${SPECIES}_${CHAIN}?${SEGMENT}.fasta \
                > $TMPDIR/imgt_${SPECIES,,}_${CHAIN,,}_${SEGMENT,,}.fasta
        done
    done
done
...
for F in $(ls *.fasta)
do
    # clean fasta, but leave gaps for changeo IgBLAST parsing
    Fwoext="${F%.*}"
	clean_imgtdb.py --gapped ${F} ${OUTDIR}/database/${Fwoext}_gapped.fasta
	#cp ${F} ${OUTDIR}/fasta/${F}
    # clean fasta, but remove gaps for building IgBLAST databases
	clean_imgtdb.py ${F} ${OUTDIR}/database/${F}
	#edit_imgt_file.pl ${F} > ${OUTDIR}/fasta/${F}
	makeblastdb -parse_seqids -dbtype nucl -in ${OUTDIR}/database/${F} \
