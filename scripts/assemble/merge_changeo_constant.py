...
def main():
    ...
    # check for long C_CALL name and trim if present
    # e.g. L00022|IGHE*02|Homo_sapiens|F|CH1+CH2+C --> IGHE*02
    if '|' in isodf.C_CALL_LONG.values[0]:
        ind = 1
    else:
        ind = 0
    isodf['C_CALL'] = isodf.C_CALL_LONG.str.split('|', expand=True)[ind]

    # desired columns to save from the constant blast results
    out_cols = ['SEQUENCE_ID', 'C_CALL', 'C_LEN', 'C_IDENT', 'C_MISMATCHES',
                'C_GAPS', 'C_SCORE', 'C_START', 'C_END']
